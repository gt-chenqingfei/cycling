...
public class ServiceStubInvocation implements Invocation {
    ...
    ServiceStubInvocation(final Context context, final Class<?> iface, final Method method, final String baseUrl, InvocationExpireListener expireListener) {
        this(context, iface, method, baseUrl, Collections.<String, String>emptyMap(), expireListener);
    }

    ServiceStubInvocation(final Context context, final Class<?> iface, final Method method, final String baseUrl, final Map<String, String> headers, InvocationExpireListener expireListener) {
...
//        final AndroidHttpClient client = AndroidHttpClient.newInstance(buildUserAgent(context), context);
    ...
    private Object invoke(final HttpClient client, final Object... args) throws InvocationException {
        ...
        if (ni != null && ni.isConnected()) {
            try {
                ...
                if (request instanceof HttpEntityEnclosingRequest) {
                    ...
                    for (int i = 0; i < bodyParams.size(); i++) {
                        StringBody stringBody = new StringBody(bodyParams.get(i).getValue(), ContentType.create("text/plain", Consts.UTF_8));
            ...
            switch (status.getStatusCode()) {
                case 200: {
                    ...
                    try {
                        ...
                        APICache.getInstance(this.context).setCachedResult(apiPath, bodyCacheParams, resultEntity);
            ...
            String entity = APICache.getInstance(this.context).getCachedResult(apiPath, bodyCacheParams);
            try {
